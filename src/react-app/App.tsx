...
function App() {
	...
	if (isPaymentOpen) return <div className="payment-page">
		...
		<main className="payment-main"><section className="payment-content"><div className="payment-intro"><p className="eyebrow">Step 4</p><h1>Choose how to pay.</h1><p>Select a payment method to complete your order from <strong>{outlets[selectedOutlet].name}</strong>.</p></div><div className="payment-layout"><div className="payment-methods" role="radiogroup" aria-label="Payment methods">{paymentMethods.map((method) => <button key={method.id} className={`payment-method ${selectedPaymentMethod === method.id ? "selected" : ""}`} onClick={() => setSelectedPaymentMethod(method.id)} role="radio" aria-checked={selectedPaymentMethod === method.id}><span className="payment-mark">{method.mark}</span><span><strong>{method.name}</strong><small>{method.detail}</small></span><span className="payment-radio" /></button>)}</div><aside className="payment-summary"><p className="eyebrow">Order total</p><strong>{money.format(total)}</strong><p>{itemCount} item{itemCount === 1 ? "" : "s"} from {outlets[selectedOutlet].name}</p><button className="place-order-button" disabled={!selectedPaymentMethod} onClick={completeOrder}>Confirm payment <span>{selectedPaymentMethod ? "→" : "Select a method"}</span></button></aside></div></section></main>
	</div>;
